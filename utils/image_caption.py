...
class ImageCaptionUtils:
    ...
    def _get_llm_provider(self, provider_id: Optional[str] = None):
        ...
        if provider_id:
            ...
            if provider:
                return provider
            logger.warning(f"[ContextPlus] 无法找到指定的提供商: {provider_id}，将尝试其他选项")
        ...
        if global_provider_id:
            ...
            if provider:
                return provider
            logger.warning(f"[ContextPlus] 无法找到全局配置的提供商: {global_provider_id}，将使用默认提供商")
    ...
    async def generate_image_caption(
        ...
        if isinstance(image, bytes):
            ...
            if image.startswith(('http://', 'https://')):
                ...
                try:
                    ...
                    logger.warning("[ContextPlus] 无法解码 Base64 图片字符串，将不计算图片哈希")
                ...
                try:
                    ...
                    logger.error(f"[ContextPlus] 无法读取本地图片文件 {image_path}: {e}")
                    return f"[无法读取图片: {image_path.name}]"
            else:
                logger.warning(f"[ContextPlus] 字符串图片源既不是有效的URL、Data URL，也不是本地文件路径: {image}")
                image_url = image # 保持原样，让后续逻辑处理
        else:
            logger.error(f"[ContextPlus] 无效的图片输入源: {type(image)}")
            return None
        ...
        if isinstance(image, bytes):
            ...
            if mime_type is None:
                logger.warning("[ContextPlus] 无法识别图片MIME类型，将默认使用 'image/jpeg'")
        ...
        # 3. 如果缓存未命中，则调用LLM
        if not provider:
            logger.warning("[ContextPlus] 无法获取任何可用的LLM提供商")
            return None

        prompt = custom_prompt or self.config.get("image_caption_prompt", "请直接简短描述这张图片")

        logger.debug(f"[ContextPlus] 准备调用LLM进行图片描述...")
        logger.debug(f"  - [ContextPlus] Provider: {provider_id or '默认'}")
        logger.debug(f"  - [ContextPlus] Prompt: '{prompt}'")
        logger.debug(f"  - [ContextPlus] Image URL: '{image_url[:100]}...'")
        ...
        if caption:
            async with self._cache_lock:
                ...
                logger.debug(f"[ContextPlus] 缓存图片描述 (key: {image_hash}): {caption}")
    ...
    async def _caption_image_with_provider(self, provider, prompt, image_urls, timeout):
        ...
        try:
            ...
            logger.warning(f"[ContextPlus] 图片转述超时，超过了{timeout}秒")
            raise e
        except ClientError as e:
            logger.error(f"[ContextPlus] 图片转述LLM请求失败 (网络错误): {e}")
            raise e
